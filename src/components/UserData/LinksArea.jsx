...
export const LinksArea = ({ links }) => {
  const { location, blog, twitter, company } = links;
  const { lightMode } = useContext(ThemeContext);

  return (
    <Container>
      {/* LOCATION */}
      <Link className={`${!location && 'unavailable'}`}>
        ...
        <Data>{location || 'Not Available'}</Data>
      </Link>

      {/* BLOG */}
      <Link className={`${!blog && 'unavailable'}`}>
        {blog ? (
          <a href={blog}>
      ...
      {/* TWITTER */}
      <Link className={`${!twitter && 'unavailable'}`}>
        {twitter ? (
      ...
      {/* COMPANY */}
      <Link className={`${!company && 'unavailable'}`}>
        <svg height='20' width='20' xmlns='http://www.w3.org/2000/svg'>
          <g fill={lightMode ? '#4b6a9b' : '#fff'}>
        ...
        <Data>{company || 'Not Available'}</Data>
      </Link>
    </Container>
  );
};
